...
class ListObjectMetadataResponse(BaseModel):
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ListObjectMetadataResponse:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in object_metadatas (list)
        _items = []
        if self.object_metadatas:
            for _item in self.object_metadatas:
                if _item:
                    _items.append(_item.to_dict())
            _dict['objectMetadatas'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ListObjectMetadataResponse:
        """Create an instance of ListObjectMetadataResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ListObjectMetadataResponse.parse_obj(obj)

        _obj = ListObjectMetadataResponse.parse_obj({
            "object_metadatas": [ObjectMetadata.from_dict(_item) for _item in obj.get("objectMetadatas")] if obj.get("objectMetadatas") is not None else None,
