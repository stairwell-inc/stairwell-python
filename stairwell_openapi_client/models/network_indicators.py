...
class NetworkIndicators(BaseModel):
    ...
    """
    ip_addresses: Optional[conlist(StrictStr)] = Field(default=None, alias="ipAddresses", description="ip_addresses is the list of IPs in the binary.")
    uninteresting_ip_addresses: Optional[conlist(StrictStr)] = Field(default=None, alias="uninterestingIpAddresses", description="Address types which are typically not useful: private, global unicast and link local multicast IPs.")
    hostnames: Optional[conlist(StrictStr)] = Field(default=None, description="hostnames is the list of Hostnames in the the binary.")
    improbable_hostnames: Optional[conlist(StrictStr)] = Field(default=None, alias="improbableHostnames", description="Hostnames which are technically valid, but are most likely noisy artifacts from static analysis.")
    private_hostnames: Optional[conlist(StrictStr)] = Field(default=None, alias="privateHostnames", description="This is a list of TLDs that are closed (or probably closed) to public registrations. These are generated by some manual analysis and data analysis from active dns data.")
